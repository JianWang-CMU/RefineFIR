...
from dataset import FaceDataset, degrade
...
if __name__ == "__main__":
    ...
    if args.size == 1024:
        ...
        decoder_ckpt = torch.load("../SOAT/stylegan2-256-550000.pt", map_location=lambda storage, loc: storage)
        # decoder_ckpt = torch.load("/shared/rsaas/mchong6/stylegan/550000.pt", map_location=lambda storage, loc: storage)
    ...
    writer_path = './runs_snapCV256png_cycle_xdj_again'
    checkpoint_path = 'checkpoint_snapCV256png_cycle_xdj_again'
    ...
    for idx in pbar:
        ...
        #l1_loss = 10*F.l1_loss(fake_img, im_hq)
        ...
        vgg_ref_loss = lpips_criterion(ds_fake_img, F.interpolate(im_warp, size=(256,256), mode='area'))
        ...
        for p in range(fake_img.size(0)):
            ...
            fake_img_lq.append(degrade(fake_img[p], *degrade_param))
        fake_img_lq = torch.stack(fake_img_lq, 0)

        #cycle_loss = 100*F.mse_loss(fake_img_lq, im_lq)
        cycle_loss = 20*lpips_criterion(fake_img_lq, im_lq).mean()
        ...
        writer.add_scalar('cycle_loss', cycle_loss, i)
        ...
        if get_rank() == 0:
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    sample = torch.cat([im_source, im_warp, im_hq, im_lq, fake_img, fake_img_lq, swap_id_im, swap_warp_im], 0)
